<?php
  ...
  if ($office_id != '4') {
    $sql_diamond = mysqli_query($con,"SELECT * FROM `diamonds` WHERE `diamond_shape_id` = '$diamond_shape' AND `office_id` = '".$office_id."' AND  diamond_type ='Fancy' AND `diamond_lot_no` LIKE 'F%' AND `diamond_status` NOT IN ('Invoiced','Deleted') ORDER BY `diamond_id` DESC");
  } else {
    $sql_diamond = mysqli_query($con,"SELECT * FROM `diamonds` WHERE `diamond_shape_id` = '$diamond_shape' AND  diamond_type ='Fancy' AND `diamond_lot_no` LIKE 'F%' AND `diamond_status` NOT IN ('Invoiced','Deleted') ORDER BY `diamond_id` DESC");
  }
